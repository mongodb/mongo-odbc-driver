...
mod integration {
    ...
    fn sql_disconnect_frees_statement_implicitly() {
        ...
        unsafe {
            ...
            // stmt isn't freed here, but it should be closed when the DBC is disconnected
    ...
    fn sql_disconnect_implicitly_closing_statment_cursors_is_safe_for_unbound_statemens() {
        ...
        unsafe {
            ...
            // Should be a no-op since the statement is not bound. This will hang if SQLDisconnect does not close the cursor
            assert_eq!(
                SqlReturn::SUCCESS,
